...
1. `auth.token` trong handshake object (khuyên dùng).
2. `Authorization` header.
3. `jwt` cookie (dành cho web clients).
Server sẽ thực hiện validation và phân quyền room dựa trên Role và Trip Ownership.
